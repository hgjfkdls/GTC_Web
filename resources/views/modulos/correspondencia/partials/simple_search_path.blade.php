<blockquote>
    <a href="{{ url()->route('correspondencia.simple_search', ['id_obra' => $response['id_obra']]) }}">Busqueda Simple</a>
    @if(count($response['data_form']) > 0)
        <span class="glyphicon glyphicon-menu-right"></span>
    @endif
    @foreach(array_keys($response['data_form']) as $id)
        @if(count($response['data_form']) == 1)
            <a href="#">{{ $response['data_form'][$id]['column' . $id] . '(\'' . $response['data_form'][$id]['pattern' . $id] . '\')' }}</a>
            <span class="badge">{{ $response['data_form'][$id]['count' . $id] }}</span>
        @elseif($id === array_first(array_keys($response['data_form'])))
            <a href="{{ url()->current() . '?' . $response['data_form'][$id]['url' . $id] }}">{{ $response['data_form'][$id]['column' . $id] . '(\'' . $response['data_form'][$id]['pattern' . $id] . '\')' }}</a>
            <span class="badge">{{ $response['data_form'][$id]['count' . $id] }}</span>
